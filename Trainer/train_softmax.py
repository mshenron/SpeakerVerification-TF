...
import time
from nets import model_softmax as model
from batch import TrainData,MultiTaskBatchManager
...
parser.add_argument('--csv', type=str, help='training Data CSV file',default='ted_train.csv')
    ...
    with T_graph.as_default():
        ...
        with tf.variable_scope(tf.get_variable_scope()):
            for i in range(num_clones):
                with tf.device(available_gpus[i]):
                    ...
                    # Network outputs.
                    prediction= model(x[i * step: (i + 1) * step],step,total_speakers)
                    label_onehot = tf.one_hot(y[i * step : (i + 1) * step], depth=total_speakers+1)
                    #prediction= model(batch_x,batch_size,total_speakers)
                    #label_onehot = tf.one_hot(batch_y, depth=total_speakers+1)
                    SOFTMAX = tf.nn.softmax_cross_entropy_with_logits(logits=prediction, labels=label_onehot)
        ...
        with tf.Session(graph=T_graph,config=tf.ConfigProto(allow_soft_placement=True,log_device_placement=False)) as sess:
            ...
            while isrunning:
                ...
                if isrunning:
                    #st=time.time()
                    _,c,summary,g= sess.run([apply_gradient_op,loss,summary_op,global_step],feed_dict={x:batch_x,y: batch_y})
